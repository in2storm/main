...
static void db_discover_evt_handler(ble_db_discovery_evt_t * p_evt)
{
    ...
    if (p_evt->evt_type == BLE_DB_DISCOVERY_COMPLETE &&
        p_evt->params.discovered_db.srv_uuid.uuid == BLE_UUID_NUS_SERVICE &&//BLE_UUID_HEART_RATE_SERVICE &&
        p_evt->params.discovered_db.srv_uuid.type == BLE_UUID_TYPE_VENDOR_BEGIN);//BLE_UUID_TYPE_BLE)
    {
        ...
        uint32_t i;
			...
			//printf("[simon] p_evt->params.discovered_db.char_coun = %d \r\n",p_evt->params.discovered_db.char_count);
        for (i = 0; i < p_evt->params.discovered_db.char_count; i++)
        {
					printf("[simon] ######### found chrac = %d ##############\r\n",p_evt->params.discovered_db.charateristics[i].characteristic.uuid.uuid);
            if (p_evt->params.discovered_db.charateristics[i].characteristic.uuid.uuid ==
                BLE_UUID_NUS_TX_CHARACTERISTIC)//BLE_UUID_HEART_RATE_MEASUREMENT_CHAR)
            {
...
LOG("[HRS_C]: Nordic UART Service discovered at peer.\r\n");
        ble_hrs_c_evt_t evt;
