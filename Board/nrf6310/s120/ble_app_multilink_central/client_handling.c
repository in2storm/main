...
#endif
//0x6E40 0001 B5A3 F393 E0A9 E50E 24DC CA9E [DB]:
// {0x9E, 0xCA, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0,
                                     //0x93, 0xF3, 0xA3, 0xB5, 0x00, 0x00, 0x40, 0x6E};
#define MULTILINK_PERIPHERAL_BASE_UUID    {{0x9E, 0xCA, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0,\
                                           0x93, 0xF3, 0xA3, 0xB5, 0x00, 0x00, 0x40, 0x6E}}  /**< Peripheral base UUID. */
#define MULTILINK_PERIPHERAL_SERVICE_UUID BLE_UUID_NUS_SERVICE                    /**< Peripheral service UUID. */
#define MULTILINK_PERIPHERAL_TX_UUID    BLE_UUID_NUS_TX_CHARACTERISTIC       /**< Peripheral characterisctics UUID. */
#define MULTILINK_PERIPHERAL_RX_UUID    BLE_UUID_NUS_RX_CHARACTERISTIC       /**< Peripheral characterisctics UUID. */
    ...
    if (p_evt->evt_type == BLE_DB_DISCOVERY_COMPLETE)
    {
        ...
        for (i = 0; i < p_evt->params.discovered_db.char_count; i++)
        {
            ...
            p_characteristic = &(p_evt->params.discovered_db.charateristics[i]);
					...
					printf("max = %d m_base_uuid_type = 0x%x uuid = 0x%x [i=%d] \r\n",p_evt->params.discovered_db.char_count, m_base_uuid_type,p_characteristic->characteristic.uuid.uuid, i);
            if ((p_characteristic->characteristic.uuid.uuid == MULTILINK_PERIPHERAL_TX_UUID)
                &&
                                              ...
                                              const api_result_t     event_result)
{
    ...
    switch (p_event->event_id)
    {
       case DM_EVT_LINK_SECURED:
           ...
           break;
       default:
				 //printf("[client_handling_dm_event_handler] unexpected msg 0x%x \r\n",p_event->event_id);
           break;
    }
...
void client_handling_ble_evt_handler(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GATTC_EVT_TIMEOUT:
            ...
            break;
        default:
					//printf("[client_handling_ble_evt_handler] unexpected msg 0x%x \r\n",p_ble_evt->header.evt_id);
            break;
    }
...
void client_handling_init(void)
{
    ...
    //nrf_gpio_range_cfg_output(8, 15); //simon
...
uint32_t client_handling_destroy(const dm_handle_t * p_handle)
{
    ...
    if (p_client->state != IDLE)
    {
            m_client_count--;
            p_client->state = IDLE;
           // nrf_gpio_pin_clear(p_handle->connection_id + LED_PIN_NO_OFFSET); //simon
    }
    else
    {
