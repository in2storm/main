...
#define SCAN_LED_PIN_NO                  30//LED_0                                          /**< Is on when device is scanning. */
...
#define TARGET_UUID                BLE_UUID_NUS_SERVICE//0x180D//  BLE_UUID_NUS_SERVICE                            /**< Target device name that application is looking for. */
...
//            nrf_gpio_pin_set(CONNECTED_LED_PIN_NO);
           ...
           // lcd_write_status = APPL_LCD_WRITE("Connected", 9, LCD_UPPER_LINE, 0);
            ...
            m_peer_count++;
            if (m_peer_count < MAX_PEER_COUNT)  //not executed...yet...this version
            {
                scan_start();
            }
            APPL_LOG("[APPL]: << DM_EVT_CONNECTION\r\n");
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_ADV_REPORT:
        {
            ...
            err_code = adv_report_parse(BLE_GAP_AD_TYPE_COMPLETE_LOCAL_NAME,//BLE_GAP_AD_TYPE_16BIT_SERVICE_UUID_MORE_AVAILABLE,
                                        &adv_data,
                                        &type_data);
				//	printf("[simon] BLE_GAP_EVT_ADV_REPORT errcode = %d \r\n",err_code);
                ...
                for (uint32_t u_index = 0; u_index < (type_data.data_len/UUID16_SIZE); u_index++)
                {
                    ...
                    if(extracted_uuid == TARGET_UUID)
                    {
                        ...
                        break;
						            ...
						            // Verify if short or complete name matches target.
            if ((err_code == NRF_SUCCESS) &&
               (0 == memcmp("nRFDuino",type_data.p_data,type_data.data_len)))
            {
                err_code = sd_ble_gap_scan_stop();
                if (err_code != NRF_SUCCESS)
                {
...
static void ble_stack_init(void)
{
    ...
    SOFTDEVICE_HANDLER_INIT(NRF_CLOCK_LFCLKSRC_RC_250_PPM_250MS_CALIBRATION, false); //NRF_CLOCK_LFCLKSRC_RC_250_PPM_250MS_CALIBRATION NRF_CLOCK_LFCLKSRC_XTAL_20_PPM
...
static void device_manager_init(void)
{
    ...
    init_param.clear_persistent_data = 1; //simon
    ...
    switch (p_hrs_c_evt->evt_type)
    {
        case BLE_HRS_C_EVT_DISCOVERY_COMPLETE:
           ...
           // success = APPL_LCD_WRITE("Heart Rate", 10, LCD_UPPER_LINE, 0);
						APPL_LOG("[simon] discovery completed...\r\n");//simon
            //APP_ERROR_CHECK_BOOL(success);
            break;
